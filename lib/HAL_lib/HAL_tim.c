...
void TIM_OC1Init(TIM_TypeDef* tim, TIM_OCInitTypeDef* pInitStruct)
{
    ...
    MODIFY_REG(tim->CCER, TIM_CCER_CC1P | TIM_CCER_CC1E, \
        pInitStruct->TIM_OCPolarity);
    MODIFY_REG(tim->CCER, TIM_CCER_CC1E, pInitStruct->TIM_OutputState);
    WRITE_REG(tim->CCR1, pInitStruct->TIM_Pulse);

    MODIFY_REG(tim->CCER, TIM_CCER_CC1NP, pInitStruct->TIM_OCNPolarity);
    MODIFY_REG(tim->CCER, TIM_CCER_CC1NE, pInitStruct->TIM_OutputNState);
...
void TIM_OC4Init(TIM_TypeDef* tim, TIM_OCInitTypeDef* pInitStruct)
{
    ...
    MODIFY_REG(tim->CCER, TIM_CCER_CC4E | TIM_CCER_CC4P,
...
{
    (state) ? SET_BIT(tim->BDTR, TIM_BDTR_MOE) : \
...
void TIM_ETRConfig(TIM_TypeDef* tim, TIMEXTTRGPSC_Typedef psc, TIMETP_Typedef \
    polarity, u16 filter)
{
    CLEAR_BIT(tim->SMCR, TIM_SMCR_ECE);
...
{
    (state) ? SET_BIT(tim->CR1, TIM_CR1_ARPE) : \
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC1PE, preload);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC2PE, preload << 8);
}
...
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_OC3PE, preload);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR2_OC4PE, preload << 8);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC1FE, fast);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC2FE, fast << 8);
}
...
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_OC3FE, fast);
}
...
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_OC4FE, fast << 8);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC1CE, clear);
}
...
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_OC2CE, clear << 8);
}
...
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_OC3CE, clear);
}
...
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_OC4CE, clear << 8);
}
...
{
    MODIFY_REG(tim->CCER, TIM_CCER_CC1E << channel, ccxEn << channel);
}
    ...
    TIMCCxNE_Typedef ccxnEn)
{
    if (channel != TIM_Channel_4)
        MODIFY_REG(tim->CCER, TIM_CCER_CC1NE << channel, ccxnEn << channel);
}
...
{
    CLEAR_BIT(tim->CCER, TIM_CCER_CC1E << channel);
    switch (channel) {
    ...
    u16 filter)
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_CC1S | TIM_CCMR1_IC1F, \
        (filter << 4) | selection);
    MODIFY_REG(tim->CCER, TIM_CCER_CC1E | TIM_CCER_CC1P, polarity |\
        TIM_CCER_CC1E);
}
    ...
    u16 filter)
{
    MODIFY_REG(tim->CCMR1, TIM_CCMR1_CC2S | TIM_CCMR1_IC2F, (filter << 12) | \
        (selection << 8));
    MODIFY_REG(tim->CCER, TIM_CCER_CC2E | TIM_CCER_CC2P, (polarity << 4) | \
        TIM_CCER_CC2E);
}
    ...
    u16 filter)
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_CC3S | TIM_CCMR2_IC3F, (filter << 4) | \
        selection);
    MODIFY_REG(tim->CCER, TIM_CCER_CC3E | TIM_CCER_CC3P, (polarity << 8) | \
        TIM_CCER_CC3E);
}
    ...
    u16 filter)
{
    MODIFY_REG(tim->CCMR2, TIM_CCMR2_CC4S | TIM_CCMR2_IC4F, (filter << 12) | \
        (selection << 8));
    MODIFY_REG(tim->CCER, TIM_CCER_CC4E | TIM_CCER_CC4P, (polarity << 12) | \
        TIM_CCER_CC4E);
}
